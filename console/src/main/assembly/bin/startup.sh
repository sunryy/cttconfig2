...
LOG_BASE_DIR=$ctgMesConfigConsole_HOME
CONFIG_DIR=""
DEBUG_MODE="0";
...
#debug Options
#JAVA_OPTS="$JAVA_OPTS -agentlib:jdwp=transport=dt_socket,server=y,suspend=n,address=7180"

# ZK
#JAVA_OPTS="$JAVA_OPTS -Djava.security.auth.login.config=/etc/security/apps_client_jaas.conf"
#==============================================================================

if [ -z "$ctgMesConfigConsole_HOME" ] ; then
    echo
    echo "Error: ctgMesConfigConsole_HOME environment variable is not defined correctly."
    echo
    exit 1
fi
#==============================================================================
...
#set CLASSPATH
ctgMesConfigConsole_CLASSPATH="$ctgMesConfigConsole_HOME/conf:$ctgMesConfigConsole_HOME/lib/*"
#==============================================================================

#startup Server
RUN_CMD="\"$JAVA_HOME/bin/java\""
RUN_CMD="$RUN_CMD -DappName=pgbackup -DctgMesConfigConsole_HOME=\"$ctgMesConfigConsole_HOME\""
RUN_CMD="$RUN_CMD -DLOG_BASE_DIR=\"$LOG_BASE_DIR\""
RUN_CMD="$RUN_CMD -Dlogging.path=\"$LOG_BASE_DIR/logs\""
RUN_CMD="$RUN_CMD -classpath \"$ctgMesConfigConsole_CLASSPATH\""
RUN_CMD="$RUN_CMD $JAVA_OPTS"
RUN_CMD="$RUN_CMD com.ctg.mes.config.console.Application $@"
RUN_CMD="$RUN_CMD >> \"$LOG_BASE_DIR/logs/console.log\" 2>&1 &"
echo $RUN_CMD
eval $RUN_CMD
echo $! > $ctgMesConfigConsole_HOME/bin/ctgMesConfigConsole.pid
#==============================================================================
